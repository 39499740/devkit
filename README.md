...
| 分类 | 内容 |
| --- | --- |
| 数据格式 | JSON、YAML、CSV、SQL、XML 等结构化数据的格式化、校验、查询与转换 |
...
**本次新增（应用内目录带「新」徽标）**：SQL 格式化、XML 工具箱、JSONPath / JMESPath 查询、JSON Schema 校验四个工具，以及 **处理流程**（流程列表 + 三栏编排与逐步运行）、跨工具「发送到…」传递。

**系统页**：首页（收藏 / 最近 / 常用 / 分类入口 + 处理流程入口）、**全部工具目录**（`/tools`，45 个工具按分类完整列出）、分类列表、全局搜索（⌘K 命令面板）、收藏、最近使用、**处理流程**（流程列表 + 三栏编排与逐步运行）、偏好设置、隐私说明、帮助、404、离线状态。

**共享交互**：结果待更新（stale）模式、文件处理反馈、剪贴板降级、跨工具内存传递（「发送到…」路由弹层）、PWA 安装入口与离线 / 新版本状态、明暗主题、响应式（桌面 / 移动）。
...
devkit/
  app/
    components/     # 共享 UI 组件（Dk* 前缀）与 tools/ 下 45 个工具页组件
...
图标是「工具箱」——`#2563EB` 圆角方块上是白色的实心箱体 + 盖缝 + 分格 + 提手。含义：箱体＝收纳容器（不绑定职业）；提手＝随身携带（本地处理、离线可用、可装成桌面应用）；分格＝各类工具各就各位。设计与生成说明见 [`docs/logo-proposal-20260921/README.md`](docs/logo-proposal-20260921/README.md)。
## 设计稿
