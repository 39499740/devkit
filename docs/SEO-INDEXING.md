...
| sitemap | URL 不带尾斜杠（`/favorites`），访问时会 302 | **62 条**（6 固定页 + 3 场景页 + 8 分类页 + 45 工具页），全部带尾斜杠，与真实 200 地址一致；另有纯文本 `sitemap.txt` 同集合兜底 |
...
2. **API 主动推送**（推荐，每次发版跑一次）：
   ...
   - ```bash
     ...
     ```
   - 脚本会把 sitemap 里的全部 URL（当前 62 条）一次性推给百度（token 只从环境变量读，不会写进仓库）
...
**2026-09-25 实测（登录态浏览器核查资源平台）**：收录尚未发生——索引量报表 09-20~09-23 全为 0，`site:` 查询零结果；抓取频次近一个月贴地（爬虫基本未主动来访）。但**抓取诊断实测成功**（10:30，Baiduspider UA 抓首页）：HTTP/2 200 / tencent-cos / 0.111 秒 / 91.6 KB，且拿到的是 09-24 新构建（entry `CzckTAX4.js`）——**站点对蜘蛛完全可达、内容是新版，纯属性等待期**。推送侧正常：API 推送 09-22 起每日 10 条（launchd），配额修复无影响。下一步：若 09-28 前后索引量仍为 0，再排查「抓取异常」报表并考虑外链加速（GitHub README / 掘金 / 知乎）。

> 平台备注：抓取诊断每次提交都可能弹旋转滑块验证码，脚本/自动化基本无法稳定通过（实测 5 次精确转正仍被拒），需人工拖动；诊断记录里 URL 填重（`https://.../https://...`）会直接抓取失败。
